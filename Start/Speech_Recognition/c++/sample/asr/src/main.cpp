...
void asr_set_config_params(bds::BDSSDKMessage &cfg_params) {
    ...
    const std::string app_id = "10677609";
    const std::string chunk_key = "cck8XUmWiGclfBcV8k7rv5Y2";
    const std::string secret_key = "zxbVOOGuiEDKHkDCxrGOFmISommL5itU";
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < THREAD_NUM; ++i) {
        ...
        snprintf(file_names[i], 256, "%s/%d.pcm", audio_dir, i); 
